...
class AgrunomProjectApplication(customtkinter.CTkFrame):
    ...
    def calculate(self, tukey=False):
        ...
        for label in items:
            ...
            Utils.append_chart_to_excel_openpy(f"Number of {label} per leaf in average",output_path,row,df_len,1,"גרפים",cropped=False, label=label)
            ...
            self.output_df.iloc[:, 1:] = self.output_df.iloc[:, 1:].apply(
                ...
                axis=1)
            row,df_len = Utils.append_df_to_excel(output_path, self.output_df, "גרפים")
            Utils.append_chart_to_excel_openpy(f"Decrease(%) of {label} as correlation to control",output_path, row + 1,df_len,2, "גרפים", cropped=True, label=label)
